...
if args.simulator == "mujoco":
    env = HumanoidEnv(cfg)
elif args.simulator in ["nimble","diffsim"]:
...
class MyVisulizer(Visualizer):
    ...
    def __init__(self, vis_file, actions = None):
        ...
        if args.simulator in ["nimble","diffsim"]:
    ...
    def data_generator(self):
        ...
        while True:
            ...
            for t in range(200):
                ...
                if args.simulator == "diffsim":
                    next_state, done = env.diff_step(torch.as_tensor(action), torch.as_tensor(state), return_done=True)
                else:    
                    next_state, reward, done, _ = env.step(action)
                if args.render_nimble:
                    env.render_nimble()
                print("t:",t)
                if running_state is not None and args.simulator != "diffsim":
    ...
    def update_pose(self):
        model = env.mj_model if args.simulator in ["nimble","diffsim"] else env.model
